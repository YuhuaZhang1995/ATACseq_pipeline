...
if all(x in func_call for x in ['alignment','adapter_trimming','filtering','plotting']):
	...
	if all(x in commands for x in ['core_info_file','seq_data','batch','run']):
		...
		#filter the reads
		in_bam=entire_output+'/gotcloud_alignment/config/metagotCloudbamfiles_Batch'+batch+'Run_'+run
		job_filter=5
		...
		#plotting
		in_bam_filtered=entire_output+'/filtering/metagotCloudbamfiles_filtered_Batch'+batch+'Run_'+run
		out_plot=entire_output+'/plot'
		os.system('mkdir '+entire_output+'/plot')
		os.system('Rscript insertsizehist.R '+in_bam+' '+entire_output+'/metagotCloudbamfiles_Batch'+batch+'Run_'+run+'.pdf')
		os.system('Rscript insertsizehist.R '+in_bam_filtered+' '+entire_output+'/metagotCloudbamfiles_filtered_Batch'+batch+'Run_'+run+'.pdf')
